...
import '../view/giftVoucher/voucharcartDataModel.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  Future<ApiResponse> addVoucharToCartAPI(VoucharCartModel cart, Function callback, BuildContext context, List<ChoiceOptions>? choices, List<int>? variationIndexes) async {
    _addToCartLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await cartRepo!.addVoucharToCartListData(cart, choices!, []);
    _addToCartLoading = false;
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _addToCartLoading = false;
      Map map = apiResponse.response!.data;
