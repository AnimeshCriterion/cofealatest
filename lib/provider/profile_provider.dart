...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<double?> getUserWalletBalance(BuildContext context)  async {
    String userID = '-1';
    ApiResponse apiResponse = await profileRepo!.getUserInfo();
    print("AnimeshCheckWallet"+apiResponse.response!.data.toString());
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _userInfoModel = UserInfoModel.fromJson(apiResponse.response!.data);
      userID = _userInfoModel!.id.toString();
      _balance = _userInfoModel!.walletBalance;
      print('===> nai kenoCheck==>${_userInfoModel!.walletBalance.toString()}');
    } else {
      ApiChecker.checkApi( apiResponse);
    }
    notifyListeners();
    return _userInfoModel!.walletBalance;
  }
